...
public class EncoderDebugger {
    ...
    private final int mBitrate, mFramerate;
    ...
    private CodecManager.Codec[] encoders;

    private EncoderDebugger(SharedPreferences prefs, int width, int height, int bitrate, int framerate, String encoderName) {
        mPreferences = prefs;
        mWidth = width;
        mHeight = height;
        mSize = width * height;
    ...
    public synchronized static void asyncDebug(final Context context, final int width, final int height, final int bitrate, int framerate, String encoderName) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
                    debug(prefs, width, height, bitrate, framerate, encoderName);
                } catch (Exception e) {
                }
            }
        }).start();
    ...
    public synchronized static EncoderDebugger debug(Context context, int width, int height, int bitrate, int framerate, String encoderName) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        return debug(prefs, width, height, bitrate, framerate, encoderName);
    }

    public synchronized static EncoderDebugger debug(SharedPreferences prefs, int width, int height, int bitrate, int framerate, String encoderName) {
        EncoderDebugger debugger = new EncoderDebugger(prefs, width, height, bitrate, framerate, encoderName);
        debugger.debug();
        return debugger;
    }
    ...
    private void debug() {
        ...
        encoders = CodecManager.findEncodersForMimeType(MIME_TYPE);
        ...
        for (int i = 0; i < encoders.length; i++) {
            Log.w(TAG, "debug: encoder" + i + " = " + encoders[i].name);
            if (mEncoderName == null || mEncoderName.length() == 0) {
                throw new NullPointerException("encoder name shouldn't be empty");
            }
            if (!encoders[i].name.contains(mEncoderName)) {
                Log.w(TAG, "debug: skip " + encoders[i].name);
                continue;
            }
